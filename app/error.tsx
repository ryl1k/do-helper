...
export default function ErrorPage({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  return (
    <div className="min-h-screen bg-canvas text-ink flex flex-col items-center justify-center px-6 py-10">
      <div className="text-bad text-[14px] uppercase tracking-wider font-semibold">Помилка</div>
      <h1 className="text-[32px] font-semibold tracking-tighter2 mt-2">Щось пішло не так</h1>
      <p className="text-[13px] text-ink-dim mt-2 max-w-md text-center break-all">
        {error.message}
      </p>
      {error.digest && (
        <p className="text-[11px] text-ink-mute mt-2">ID: {error.digest}</p>
      )}
